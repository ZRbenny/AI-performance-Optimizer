...
from ai_advisor import generate_ai_report
...
if "refresh_rate" not in st.session_state:
    st.session_state.refresh_rate = 15  # default 15s

# Toggle for auto-refresh
st.sidebar.header("⚙️ Settings")
st.session_state.auto_refresh = st.sidebar.checkbox(
    "Enable Auto-Refresh", value=False
)
...
save_metrics(metrics)  # always record once at load
...
if st.button("Run AI Analysis"):
    ai_report = generate_ai_report(history_limit=20)
    st.write(ai_report)
